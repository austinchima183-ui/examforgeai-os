#!/bin/bash
# ============================================================================
# HARDENING RELEASE — deploy EXACTLY commit 2e5fdc3 to production
...
#         deploy-879827e.sh / deploy-54cb1c0.sh)
# ============================================================================
set -u
# Vercel token is NEVER hardcoded — provide at runtime:
#   VERCEL_TOKEN=vcp_... ./deploy-2e5fdc3.sh
VERCEL_TOKEN="${VERCEL_TOKEN:?VERCEL_TOKEN env var required (Vercel personal access token)}"
TEAM="team_hbVXkzeMbXEmG1e6FO4tq4vB"
CERTIFIED_SHA="2e5fdc31997d7780c43b5ccfdcd6fa30e2ecb261"
REPO="austinchima183-ui/examforgeai-os"
OUT="/tmp/deploy-trigger-2e5fdc3.json"
...
curl -s -H "Authorization: Bearer $VERCEL_TOKEN" "https://api.vercel.com/v13/deployments/$DEP_ID?teamId=$TEAM" -o /tmp/deploy-final-2e5fdc3.json
python3 -c "
import json
d=json.load(open('/tmp/deploy-final-2e5fdc3.json'))
print('readyState:',d.get('readyState'))
print('errorMessage:',d.get('errorMessage'))
meta=d.get('meta',{})
print('meta.githubCommitSha:',meta.get('githubCommitSha'))
print('COMMIT MATCH:', 'YES' if str(meta.get('githubCommitSha','')).startswith('2e5fdc3') else 'NO — MISMATCH')
