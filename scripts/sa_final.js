...
(async () => {
  ...
  const dom = await page.evaluate(() => ({
    ...
    mainText: (document.querySelector('main')?.innerText || '').slice(0, 300).replace(/\n+/g, ' | '),
  }));
  console.log(`URL: ${url}`);
  console.log(`h1: ${dom.h1}`);
  console.log(`text: ${dom.text}ch | glass: ${dom.glass} | cards: ${dom.cards}`);
  console.log(`main: ${dom.mainText.slice(0, 200)}`);
  await browser.close();
})();
